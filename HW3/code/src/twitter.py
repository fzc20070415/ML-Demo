...
def extract_dictionary(infile):
    ...
    with open(infile, 'rU') as fid :
        ...
        i = 0
        for line in fid:
            for each_word in extract_words(line):
                if each_word not in word_list:
                    word_list[each_word] = i
                    i=i+1
        ### ========== TODO : END ========== ###
        ...
        for line in fid:
            for each_word in extract_words(line):
                if each_word in word_list:
                    feature_matrix[i, word_list[each_word]]=1
            i=i+1
...
def cv_performance(clf, X, y, kf, metric="accuracy"):
    ...
    for x1, x2 in kf:
        ...
        sum += performance(y_test, clf.decision_function(X_test), metric)
        i += 1
    return sum/i
    ### ========== TODO : END ========== ###
...
def select_param_linear(X, y, kf, metric="accuracy"):
    ...
    print("metric: " + metric)
    for i in C_range:
        i_perf = cv_performance(SVC(kernel='linear', C=i), X, y, kf, metric)
        if i_perf >= opt_perf:
            opt = i
            opt_perf = i_perf
        print ("c: ", end=" ")
...
def main() :
    ...
    test_data_X = X[560: , : ]
    test_data_y = y[560: ]
    # print("Shape of train_data_X is:", end=" ")select_param
    # print(train_data_X.shape)
    # print("Shape of train_data_y is:", end=" ")
    # print(train_data_y.shape)
    # print("Shape of test_data_X is:", end=" ")
    # print(test_data_X.shape)
    # print("Shape of test_data_y is:", end=" ")
    # print(test_data_y.shape)

    # part 2: create stratified folds (5-fold CV)
    skf = StratifiedKFold(train_data_y, n_folds=5)
    # part 2: for each metric, select optimal hyperparameter for linear-kernel SVM using CV

    for metric in metric_list:
        met_perf = select_param_linear(train_data_X, train_data_y, skf, metric)

    # part 3: train linear-kernel SVMs with selected hyperparameters
    clf = SVC(C=10**(1), kernel='linear')
    clf.fit(train_data_X, train_data_y)
    # part 3: report performance on test data
    for metric in metric_list:
        print("Performance for metric " + metric + " is: \t", end=" ")
        print(performance_test(clf, test_data_X, test_data_y, metric))
    ### ========== TODO : END ========== ###
