...
class PolynomialRegression() :
    ...
    def generate_polynomial_features(self, X) :
        ...
        if m==1:
            temp_a = np.ones((n,1))
            Phi = np.hstack((temp_a, X))

        else:
            """
            import sklearn.preprocessing as sk
            #X is a N*1 vector
            poly = sk.PolynomialFeatures(m)
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in range(tmax) :
            ...
            self.coef_ -= 2*eta*np.dot(np.transpose(X), y_pred - y)
            ### ========== TODO : END ========== ###
    ...
    def fit(self, X, y, l2regularize = None ) :
        ...
        self.coef_ = np.dot(np.linalg.pinv(np.dot(X.transpose(), X)), np.dot(X.transpose(), y))
        ### ========== TODO : END ========== ###
    ...
    def cost(self, X, y) :
        ...
        #cost = 0
        cost = np.dot(self.predict(X)-y, self.predict(X)-y)
...
def main() :
    ...
    print('Investigating gradient descet...')
    model = PolynomialRegression()
    for eta in [0.0001, 0.001, 0.01, 0.0407]:
        t1 = time.time()
        model.fit_GD(train_data.X, train_data.y, eta=eta)
        t2 = time.time()
        print('Coefficient: ', eta)
        print('Final value: ',model.coef_)
    ...
    print('Investigating gradient descet with decreasing learning rate ...')
    model = PolynomialRegression()
    t1 = time.time()
    ...
    x_x = np.arange(0,11)
    for x in x_x:
        pmodel = PolynomialRegression(m=x)
        pmodel.fit(train_data.X, train_data.y)
        rms_train.append(pmodel.rms_error(train_data.X,train_data.y))
        rms_test.append(pmodel.rms_error(test_data.X,test_data.y))

    plt.plot(x_x, rms_train, '-r.', label="RMSE-train_data")
    plt.plot(x_x, rms_test, '-b.', label="RMSE-test_data")
    plt.xlabel("m")
    plt.ylabel("RMSE")
    plt.legend()
    plt.show()
